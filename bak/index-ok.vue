...
import {
    computed, defineComponent, onMounted, nextTick, reactive, provide, ref
} from 'vue';
import {
    renderSlot,
    funCall,
...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        const state = reactive({
            ...
            // currentMessages: [],
            messagesHistoryVisible: false,
            activeSidebar: DEFAULT_MENU_LASTMESSAGES
        });

        const propsContacts = ref([]);
        let menus = reactive([]);
        ...
        const menuIsContacts = () => state.activeSidebar === DEFAULT_MENU_CONTACTS;
        const menuIsMessages = () => state.activeSidebar === DEFAULT_MENU_LASTMESSAGES;

        const isContactContainerCache = name => name.startsWith('contact#');
        ...
        // 返回当前聊天窗口联系人信息
        const getCurrentContact = () => currentContact.value;
        ...
        /**
         * 根据 id 查找消息的索引
         * @param contactId 消息 id
         * @return {Number} 消息索引，未找到返回 -1
         */
        const findMessageIndexById = messageId => currentMessages.value.findIndex(item => item.id === messageId);
         ...
         * @return {Boolean}
         */
        const hasContact = contactId => findContactIndexById(contactId) !== -1;
        ...
        const findCurrentMessage = (messageId) => {
        ...
        const updateMessage = (message) => {
            ...
            // const index = findMessageIndexById(message.id);
            // let historyMessage = findCurrentMessage(message.id);
            // if (!historyMessage) return false;
            // historyMessage = Object.assign(historyMessage, message, {
            //     toContactId: historyMessage.toContactId
            // });
            // allMessages[state.currentContactId].splice(index, 1, message);
            // debugger
        ...
        const updateCurrentMessages = () => {
            if (!allMessages[state.currentContactId]) { allMessages[state.currentContactId] = []; }
            // if (messagesList.length > 0) {
            //     currentMessages.value = [...messagesList];
            // } else {
            //     currentMessages.value = [...allMessages[state.currentContactId]];
            // }
            currentMessages.value = [...allMessages[state.currentContactId]];
            // currentMessages.value = allMessages[state.currentContactId];
            // state.currentMessages = allMessages[state.currentContactId];
            // debugger
        };

        // watch(lastMessages, () => {
        //     console.log('---------------------lastMessages-----------------------')
        //     console.log('lastMessages.value:', lastMessages.value.length, JSON.stringify(lastMessages.value))
        // })
        // watch(currentMessages, () => {
        //     console.log('---------------------currentMessages-----------------------')
        //     console.log('currentMessages.value:', currentMessages.value.length, JSON.stringify(currentMessages.value))
        // })

        const getCurrentMessages = () => currentMessages.value;
        ...
        const _addMessage = (message, contactId, t) => {
            ...
            allMessages[contactId][type](...message);
            currentMessages.value = [...allMessages[contactId]];
            // allMessages[contactId][type](...messages.value);
            // debugger
        };
        ...
        const _emitPullMessages = (next) => {
            ...
            emit(
                ...
                (messagesList = [], isEnd = false) => {
                    ...
                    updateCurrentMessages(messagesList);
         ...
         */
        const getMessages = contactId => (contactId ? allMessages[contactId] : allMessages) || [];
        ...
        const handleToggleSidebar = (e) => {
            ...
            state.messagesHistoryVisible ? state.messagesHistoryVisible = false : state.messagesHistoryVisible = true;
        };
        ...
        const renderRightSidebar = (currContact) => {
            if (currContact.isGroup && !state.messagesHistoryVisible) {
                return (
                    renderSlot(slots['message-side'], null, currContact)
                );
            }
            if (state.messagesHistoryVisible) {
                return renderSlot(
                    slots['message-sidebar'],
                    <div class="witalk-right-messages" style="width: 300px;">
                        消息记录
                    </div>,
                    currContact
                );
        ...
        const _renderContainer = () => {
            ...
            // debugger
            nodes.push(
                <div class={cls} v-show={menuIsMessages() && defIsShow && currContact.id}>
                    ...
                    <div class="witalk-vessel">
                        <div class="witalk-vessel-left">
                            ...
                            <witalk-editor
                                ref={editorRef}
                                tools={editorTools.value}
                                sendText={props.sendText}
                                sendKey={props.sendKey}
                                onSend={_handleSend}
                                onUpload={_handleUpload}
                                emojiData={attrs.emojiData}
                        ...
                        <div class="witalk-vessel-right">
                            {renderRightSidebar(currContact)}
                        </div>
                    </div>
                </div>
            );
            ...
            nodes.push(
                <div
                    class={cls}
                    v-show={menuIsContacts() && defIsShow && currContact.id}
                >
                {renderSlot(
                    slots['contact-info'],
        ...
        const _renderDrawer = () => (menuIsMessages() && state.currentContactId ? (
